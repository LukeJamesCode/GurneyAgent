...
<p align="center">
  ...
  <a href="./CHANGELOG.md"><img alt="Version" src="https://img.shields.io/badge/version-0.7.0-green.svg" /></a>
...
- **Doesn't phone home** — no API keys to a third party for the chat loop, no usage analytics, no "free tier" that becomes a paid tier next quarter.
...
| `gurney-websearch`           | Web search as an LLM tool (DuckDuckGo / SearXNG)                      |
| `gurney-memgraph`            | Long-term memory via FalkorDB (heavy tier only)                       |
...
| --------- | ----------------------------- | ----------------------------------------- |
| Light     | Pi 4 (4 GB), Pi 5 (4 GB)      | Core + everyday-assistant + websearch     |
| Standard  | Pi 5 (8 GB), N100 mini PC     | Above + TTS + routines                    |
| Heavy     | 5800H / Ryzen mini PC, 16 GB+ | Above + memgraph (long-term memory)       |
...
**v0.7** — feature-complete; running through the [release checklist](./docs/release-checklist.md) toward 1.0. The host API for extensions is considered stable; breaking changes will be called out in [CHANGELOG.md](./CHANGELOG.md) with a deprecation window.
