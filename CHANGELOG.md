...
- **Attach files, folders, images, and PDFs to chats and workflow runs too — a `+` on every prompt.** The task-attachment feature now reaches the two remaining surfaces, each with an "add files" button in the `gurney-frontend` panel. The `+` opens a small menu to add loose **files** or a whole **folder** (structure preserved), shared across the chat bar, workflow run bar, and the Agents Launch composer. **Chat:** the chat bar gains a `+` (files/folders/images/PDFs); images ride the turn for a vision-capable chat model, while file and PDF text is extracted and inlined into the prompt (capped per file so a big upload can't blow a Pi's context), all turn-scoped and never persisted. **Workflows:** the run bar gains a `+`; the uploaded batch is ingested into **each agent node's task** as it runs — so the workflow's agents see images and `read_file`/`list_dir` pin to the files, reusing the whole agent-attachment path. New migration `0019_workflow_run_attachments.sql` (a `stage_token` on `workflow_runs`); core gains a registry-free `readStagedAttachments` (for chat) plus a `keepStaging`/`removeStagedBatch` pair so one run can feed several agent nodes from one staged batch. All three surfaces stage through the same generic upload endpoint and a shared `useAttachments` panel hook.
- **Drop files, folders, images, and PDFs into an agent task.** A dispatch can now carry inputs instead of a bare prompt. Text/code files and whole folders are saved to a per-task directory that becomes the `read_file`/`list_dir` root for that run (structure preserved, no context blow-up — it scales to a repo); PDFs are text-extracted into that dir (pure-JS `unpdf`, no system deps, Pi-safe); images are fed to the model as base64. Images and PDFs are **gated on a multimodal model** (Ollama `/api/show` vision capability, with a `qwen3`/`gemma≥4` family fallback) — a text-only agent refuses them with a note rather than feeding a blind model. Two surfaces: the **panel's dispatch composer** gains a drop-zone (files + folders, per-file chips, blocked visuals flagged per the agent's model), and **Telegram** accepts a photo or document whose caption is `/dispatch <agent> <task>`. Core plumbing only: vision wiring in the LLM layer, migration `0018`, `src/core/agent-attachments.ts`; the web UI stays in the `gurney-frontend` extension.
- **Opt-in read-only filesystem tools for agents (`read_file` / `list_dir`).** Set `GURNEY_FS_ROOT` to a directory (e.g. a local checkout) and agents gain two read-only tools pinned to that root — so a code-review or local-analysis agent can actually read code instead of hallucinating a `cat` tool core never shipped. Off by default (no `GURNEY_FS_ROOT` ⇒ not registered, no new attack surface). Access is hard-pinned: `..` traversal, absolute paths, and symlinks pointing outside the root are all refused (realpath-checked); reads are size-capped, listings entry-capped, and binary files are refused rather than dumped as mojibake. New `src/core/fs-tools.ts`.
- **Autonomous agents (long-horizon plan→act→reflect loop).** An agent can now be set to `autonomous` mode and handed a hard goal: it authors a plan, works it step by step over many turns, and finishes on its own — the foundation for "run for an hour" tasks. The loop is deterministic code; the model drives it through five built-in tools visible only to autonomous agents (`update_plan`, `complete_step`, `record_finding`, `save_artifact`, `finish` — new `src/core/agent-planning.ts`). Stopping is enforced in code (on `finish`, plan-complete, a round/wall-clock **budget**, or a stall), never left to the model. New migration `0017_agent_autonomy.sql`; a flagship **operator** persona is seeded.
- **Durable resume + cooperative pause/steer.** Autonomous runs checkpoint after every step, so a daemon restart **resumes from the checkpoint** instead of replaying the goal (wall-clock budget preserved); single-mode tasks still re-run from scratch. Pausing stops a run **between steps** with state intact, and you can **steer** a running agent — inject guidance applied before its next step — without cancelling it.
- `gurney-frontend`: a **live agent run view**. Opening a task now streams it over SSE (`/api/agents/tasks/:id/stream`) with a budget gauge, the plan ticking off, saved **artifacts**, the transcript, and a **Steer** box. The agent editor exposes the run mode + budgets; autonomous agents carry a badge. New `/api/agents/tasks/:id/{stream,steer,artifacts}` routes.
...
- **Autonomous loop: a stalled agent can no longer grind to its budget by re-wording its plan.** The stall guard compared the whole plan JSON between turns, so a model that re-authored its plan every turn (different step titles) without ever completing a step reset the no-progress counter each time and ran all the way to the round/wall-clock budget (observed: ~70 min). The guard now keys on completed-step count (or `finish`), so a run making no real progress stops after `AUTONOMOUS_STALL_LIMIT` turns as intended.
- `gurney-frontend`: **the workflow builder now produces runnable tool/condition/loop nodes.** The canvas editor wrote config keys the runner never read (`toolName`/`argsTemplate`, `operator`, `bodyNodeType`/`bodyNodeConfig`), so those three node types silently did nothing; it now writes the keys the runner expects (`tool`/`args`, `op`, `body.{type,config}`). The two `window.prompt()` calls (run input and condition branch) are gone — run input is a proper field and condition branches auto-label true/false.
