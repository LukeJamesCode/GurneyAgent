...
// Combine N AbortSignals into one. Aborts when any input aborts. Avoids the
// allocation of an extra AbortController in the common single-signal path.
function composeAbort(...signals: AbortSignal[]): AbortSignal {
  const ctl = new AbortController();
  const onAbort = (s: AbortSignal): void => ctl.abort(s.reason);
