...
Web search for Gurney as a **safe, read-only** capability — exposed as the
`web_search` LLM tool and a `/search` command, and used by `gurney-tudor` to
...
- **Tudor:** enable "Research the web first" in the Learn tab (or
  `gurney config gurney-tudor` → `use_websearch`) and a new course is seeded
