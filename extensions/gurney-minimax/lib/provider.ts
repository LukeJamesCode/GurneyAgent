    ...
    chat: async function* (opts: LLMProviderChatOptions) {
      ...
      if (ceiling > 0) {
        ...
        if (usedTokens >= ceiling) {
          ...
          throw new Error(`Daily MiniMax token budget reached (${usedTokens}/${ceiling}). Set a higher ceiling via \`gurney config gurney-minimax\`.`);
        }
      }
      ...
      try {
        ...
        if (e instanceof MinimaxApiError && e.status === 401) {
          throw new Error('MiniMax API Key rejected (401). Re-run `gurney auth gurney-minimax`.');
        }
        throw e;
      }
    },
