  ...
  try {
    for (;;) {
      ...
      while ((nl = buffer.indexOf('\n')) !== -1) {
        ...
        if (parsed.base_resp && parsed.base_resp.status_code !== 0 && parsed.base_resp.status_code !== undefined) {
          throw new Error(`MiniMax API error: ${parsed.base_resp.status_msg} (${parsed.base_resp.status_code})`);
        }
        ...
        if (parsed.usage) {
          if (parsed.usage.prompt_tokens !== undefined) chunk.promptTokens = parsed.usage.prompt_tokens;
          if (parsed.usage.completion_tokens !== undefined) chunk.completionTokens = parsed.usage.completion_tokens;
        }
