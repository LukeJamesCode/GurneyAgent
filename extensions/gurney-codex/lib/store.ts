...
//
// Secrets live in Gurney's `extension_settings` table (same place every other
// extension keeps its OAuth material), reached through `host.settings`. This
// keeps Codex auth inside ~/.gurney with the rest of the user's secrets — NOT
// in ~/.codex — so `gurney` owns the lifecycle and a future `gurney backup`
...
export class CodexNotAuthedError extends Error {
  constructor() {
    super('Codex is not configured. Run `gurney auth gurney-codex` first.');
...
// endpoint and clobber each other's rotated refresh_token. Gurney is a single
