...
function formatTools(tools: readonly ToolSchema[] | undefined): string {
  ...
  return (
    ...
    'If no tool is needed, answer normally. You cannot execute tools yourself; this JSON asks Gurney to run them locally.'
  );
}
...
function composeModelPrompt(opts: LLMProviderChatOptions): string {
  return (
    'Continue this Gurney conversation. Use the messages exactly as the conversation context, ' +
...
async function callAsModel(host: Host, opts: LLMProviderChatOptions): Promise<CodexResult> {
  ...
  if (used >= cfg.ceiling) {
    ...
    throw new Error(
      `Daily Codex budget reached (${cfg.ceiling}/${cfg.ceiling} calls used today). It resets at local midnight, or raise it with \`gurney config gurney-codex\`.`,
    );
  }
  ...
  try {
    ...
    if (e instanceof CodexApiError && e.status === 401) {
      throw new Error('Codex rejected the credentials (401). Re-run `gurney auth gurney-codex`.');
    }
    throw e;
  }
}
