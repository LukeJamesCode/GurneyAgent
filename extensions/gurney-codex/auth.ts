// `gurney auth gurney-codex` — connect a ChatGPT subscription via OAuth.
//
...
//     back into the terminal. This is the common case for Gurney's target host.
//
...
export function register(host: Host): void {
  host.auth.flow({
    ...
    run: async (io) => {
      io.print(
        'Connect Gurney to Codex using your ChatGPT Plus/Pro subscription.\n' +
      ...
      if (!accountId) {
        io.print(
          '  ⚠ Could not read a ChatGPT account id from the token. Codex calls may be rejected; ' +
            'if so, re-run auth or set codex_account_id via `gurney config gurney-codex`.',
        );
      }
      ...
      if (probe.ok) {
        ...
        io.print(
          `  ⚠ Auth completed but the test call failed (${probe.status}): ${probe.detail}\n` +
            ...
            '    Credentials were still saved; fix with `gurney config gurney-codex` and try `/codex hello`.',
        );
      }
