...
export function isDiscordChatId(chatId: number): boolean {
  return (
    ...
    chatId <= DISCORD_CHAT_ID_BASE &&
    chatId >= DISCORD_CHAT_ID_MIN_BOUND
  );
}

// Stable 39-bit hash of "<userId>:<channelId>" offset into the negative
// namespace. Deterministic so a pair always resolves to the same Gurney
...
function hashedSyntheticId(userId: string, channelId: string): number {
  ...
  // safe-int range.
  const n =
    h[0]! * 2 ** 32 + h[1]! * 2 ** 24 + h[2]! * 2 ** 16 + h[3]! * 2 ** 8 + h[4]!;
...
export interface IdentityStore {
  // Resolve or create the synthetic Gurney chatId for a Discord pair.
...
export function createIdentityStore(db: DB): IdentityStore {
  const selectByPair = db.prepare(
    `SELECT gurney_chat_id, discord_user_id, discord_channel_id, is_dm,
  ...
  const selectByChatId = db.prepare(
    `SELECT gurney_chat_id, discord_user_id, discord_channel_id, is_dm,
            first_seen_at, last_seen_at
       FROM discord_chats
      WHERE gurney_chat_id = ?`,
  );
  const selectByChatIdOnly = db.prepare(
    `SELECT gurney_chat_id FROM discord_chats WHERE gurney_chat_id = ?`,
  );
  const insert = db.prepare(
    `INSERT INTO discord_chats
       (gurney_chat_id, discord_user_id, discord_channel_id, is_dm,
        first_seen_at, last_seen_at)
       VALUES (?, ?, ?, ?, ?, ?)`,
  );
  const touch = db.prepare(
    `UPDATE discord_chats SET last_seen_at = ? WHERE gurney_chat_id = ?`,
  );
  const countAll = db.prepare(`SELECT COUNT(*) AS n FROM discord_chats`);

  function rowToRecord(r: {
    gurney_chat_id: number;
  ...
  return {
    chatIdFor({ userId, channelId, isDm }): number {
      ...
      const now = Date.now();
      if (existing) {
        touch.run(now, existing.gurney_chat_id);
        return existing.gurney_chat_id;
      }
      ...
      for (let i = 0; i < 64; i++) {
        ...
        if (candidate < DISCORD_CHAT_ID_MIN_BOUND) {
          throw new Error(
            'gurney-discord: ran out of synthetic chat-id space (BASE..MIN_BOUND exhausted)',
          );
        }
      }
