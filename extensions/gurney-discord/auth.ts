// `gurney auth gurney-discord` — paste a Discord bot token.
//
...
// makeSettings in src/core/extensions.ts), masked in the TUI by virtue of
// the `secret: true` flag in settings.schema.json. There is no `host.secrets`
// surface in v1, so this is the same pattern gurney-codex uses for OAuth
// tokens.
...
export function register(host: Host): void {
  host.auth.flow({
    ...
    run: async (io) => {
      io.print(
        'Authorize Gurney to act as a Discord bot.\n\n' +
          ...
          '  5. Enable "Message Content Intent" on the Bot page so DMs/mentions reach Gurney.\n' +
      ...
      if (token.length < MIN_TOKEN_LENGTH) {
        throw new Error(
          `That doesn't look like a Discord bot token (${token.length} chars). ` +
            'A token is typically 70+ chars; re-run `gurney auth gurney-discord` once you have it.',
        );
      }
      ...
      // entrypoint will exercise the token on next `gurney start`.
      io.print('  ✓ Token captured. Run `gurney start` (or restart) to bring the bridge online.');
