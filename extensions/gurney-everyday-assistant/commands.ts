// All 13 Telegram slash-commands for gurney-everyday-assistant.
...
import { buildMorningBrief, buildNightBrief, rememberBriefingChat } from './gather.js';
...
export function register(host: Host): void {
  ...
  host.telegram.command(
    ...
    async (ctx) => {
      ...
      const events = await c.listEvents(todayRangeIso());
      await ctx.reply(
        events.length === 0
          ? 'No events today.'
  ...
  host.telegram.command(
    ...
    async (ctx) => {
      ...
      if (!loc) {
        await ctx.reply(
          'Usage: /weather <city>\nOr set default_location in settings: gurney config',
        );
        return;
      }
