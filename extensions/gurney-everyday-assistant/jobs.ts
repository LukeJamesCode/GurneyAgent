...
async function learnedRoutineSweep(
  ...
  settings: import('../../src/core/extensions.js').ExtensionSettings,
): Promise<Nudge[]> {
  ...
  const info = db
    .prepare(
      `INSERT OR IGNORE INTO routine_rules
         (pattern_key, chat_id, title, cron, text, source_extensions, confidence, evidence_json, status, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?)`,
    )
    .run(
      c.patternKey,
