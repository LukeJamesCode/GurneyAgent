-- gurney-everyday-assistant 0003_learned_routines
-- Adopt learned-routine behavior from the (now-removed) gurney-routines extension.
...
-- Drop the old gurney-routines tables if a previous install left them behind.
...
-- Clean up settings rows from the old extension so `gurney config` doesn't
-- list ghosts. Matches the pattern used by 0001_adopt_existing_tables.
DELETE FROM extension_settings WHERE extension = 'gurney-routines';
