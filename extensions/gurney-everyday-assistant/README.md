...
2. Run `gurney auth gurney-everyday-assistant` and paste the client ID and secret.
3. Run `gurney config` → `gurney-everyday-assistant` to set `default_location` and `time_zone`.

See [docs/extensions/gurney-everyday-assistant.md](../../docs/extensions/gurney-everyday-assistant.md) for the full reference.
...
| `google_refresh_token` | —               | Set by `gurney auth` (secret)                          |
...
- OAuth credentials in `extension_settings` (plaintext SQLite protected by `~/.gurney` file permissions and masked in Gurney UI/status output)
