-- gurney-voice 0001_init: per-chat voice preference. One row per chat the user
...
-- `gurney-tts`, which created the same table under its own migration tracker.
-- A user upgrading would otherwise hit "table tts_chat_prefs already exists"
-- the first time gurney-voice's fresh migration tracker tries to run this.
