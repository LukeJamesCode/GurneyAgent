# gurney-openai-compatible

One Gurney extension for any provider that exposes an OpenAI-style
`POST <baseURL>/chat/completions` API.

This is Gurney's opt-in cloud or self-hosted escalation path. It does not change
...
Gurney's current settings UI stores scalar values, so `endpoints` is a JSON
...
Those settings live under `~/.gurney/` with the rest of Gurney's extension
state. They are not printed by `/oaiendpoints`.

## Network Allowlist

Gurney v1 manifests declare capabilities as strings; they do not yet support
...
| Ollama `/v1/chat/completions` | `http://localhost:11434/v1` | Experimental OpenAI compatibility; Gurney core already uses Ollama natively, so use this only for aliasing/proxy experiments. |
...
- Tool support is only as good as the provider/model/template. When unsure, set
  `"tools": false` and let Gurney use the JSON-envelope fallback.
