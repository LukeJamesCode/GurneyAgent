...
export function assertWithinBudget(
  ...
  if (
    ...
    throw new Error(
      `Daily budget reached for ${args.endpointAlias}: ${used.calls}/${args.dailyCallLimit} calls used today. It resets at local midnight, or raise the endpoint limit with \`gurney config gurney-openai-compatible\`.`,
    );
  }
  if (
    ...
    throw new Error(
      `Daily budget reached for ${args.endpointAlias}: ${used.totalTokens}/${args.dailyTokenLimit} tokens used today. It resets at local midnight, or raise the endpoint limit with \`gurney config gurney-openai-compatible\`.`,
    );
  }
}
