...
function ensureAllowed(endpoint: EndpointConfig, settings: OpenAICompatSettings): void {
  if (!settings.allowedBaseURLs.includes(endpoint.baseURL)) {
    throw new Error(
      `${endpoint.alias} baseURL ${endpoint.baseURL} is not in this extension install's network allowlist. Re-run \`gurney ext update gurney-openai-compatible\` or intentionally add it to allowed_base_urls to widen capabilities.`,
    );
  }
}
